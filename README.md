...
Since there's no such thing as a - hot, dry, but also sometimes rainy subtropical semi-arid-herbaceous-composite-dominated steppe biome; African Marigold spawns in the `savanna` in vanilla minetest game - and `Savanna` in mineclonia ).Because it's the closest approximation to where it would be found naturally.

Mods such as `ebiomes`, `ethereal`, and `everness` add additional secondary and tetriary biome types.
If these mods are installed flower placement will also consider the biomes provided by these mods.

If `ebiomes` is installed, *African Marigold* will spawn in:
* `savanna`
* `humid_savanna` 

If both `ebiomes` and `ethereal` are installed:
* `savanna`
* `humid_savanna`
*  `mesa`
*  `baobab_savanna`
